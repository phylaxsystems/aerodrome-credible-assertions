...
abstract contract AerodromeVeSafeHelpers is Assertion {
    ...
    constructor(address safe_, address ve_, address voter_, address protocolGovernor_, address epochGovernor_) {
        ...
        _registerAssertionSpec();
    }

    function _registerAssertionSpec() private {
        // The current local runner expects a mutable CALL for spec recording.
        // Spec 2 (Experimental) is used because this assertion relies on getLogs() and
        // getAllCallInputs() precompiles, which are not enabled under the Reshiram spec
        // in current local runner builds.
        address specRecorder = address(uint160(uint256(keccak256("SpecRecorder"))));
        (bool ok,) = specRecorder.call(abi.encodeWithSignature("registerAssertionSpec(uint8)", uint8(2)));
        require(ok, "AerodromeVeSafe: spec registration failed");
    }
